...
class SeqInstr(Instr):
    ...
    def weakest_precondition(self, postcondition):
        return self.p1.weakest_precondition(self.p2.weakest_precondition(postcondition))
...
class CondInstr(Instr):
    ...
    def weakest_precondition(self, postcondition):
        return z3.If(self.condition, self.ptrue.weakest_precondition(postcondition), self.pfalse.weakest_precondition(postcondition))
...
if __name__ == '__main__':
    ...
    # le programme à vérifier est stocké dans l'objet 'analyzer'

    solver = z3.Solver()

    # Ajouter les pré-conditions comme hypothèses dans le solveur
    for precondition in analyzer.preconditions:
        solver.add(precondition)

    # Calculer la pré-condition la plus faible à partir du corps de la méthode
    wp = analyzer.method_body.weakest_precondition(z3.And(analyzer.postconditions))

    # Vérifier que les pré-conditions impliquent la pré-condition la plus faible
    solver.add(z3.Not(wp))

    if solver.check() == z3.sat:
        print("Le programme peut ne pas être correct par rapport à sa spécification.")
    else:
        print("Le programme est correct par rapport à sa spécification.")